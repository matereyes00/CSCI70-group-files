...
// initialize required components
	...
	fgets(client_name,255,stdin);
	printf("your name is %s", client_name);
	...
	n = read(sockfd,opponent_name,255);
     		if (n < 0)
...
//
    ...
    if (n < 0) 
	  ...
	  if (flag > 0)
	  {
			...
			{
				printf("please notify %s that you already quit: ", opponent_name);
				fgets(buffer, 255, stdin);
				n = write(sockfd,buffer,strlen(buffer));
    			if (n < 0) 
         	error("ERROR writing to socket");
    ...
    choice = atoi(buffer);
	 ...
	 textcolor(1,4,8);
    printf(":: %s:: dropped an 'X' token in column -> %s \n",opponent_name, buffer);
	 textcolor(0,0,8);
